...
class KpiHelpers
{
	public static function getAnswerReport($event, $userId = null, array $areaIds = [], $from = null, $to = null)
	{
		...
		$answers = self::getUserEventAnswers($event, $userId, null, $from, $to)
			->get()
			->toArray();
			...
			foreach ($survey['questions'] as $question) {
				if (in_array($question['type'], $reportQuestionType)) {
					if ($question['type'] == 'dropdown' || $question['type'] == 'radio' || $question['type'] == 'checkboxes') {
						array_push($results, self::getAnswerReportData($question, $answers));
					} else if ($question['type'] == 'number_sales') {
						$results = array_merge($results, self::getNumberSalesData($question, $answers, $from, $to, $chartDateFormat, $mysqlDateFormat));
					}
				}
			}
		}
	...
	public static function getReportPerSalesArea($event, array $areaIds = [], $from, $to)
	{
		$results = [
			'type' => 'bar',
			'data' => [
		...
		if (count($areaIds) > 0) {
			$salesAreas = $salesAreas->whereIn('id', $areaIds);
		}
	...
	public static function getUserEventAnswers($event, $userId = null, array $areaIds = [], $from = null, $to = null)
	{
		// format for $date "Y-m-d" ex: "1975-05-21"
		$fromDate = self::getDateFromFormat($from);
		$toDate = self::getDateFromFormat($to);
	...
	private static function createChartData($type, $data, $config = null, $labels = null)
	{
		$result = [
			'type' => $type,
			'data' => [
	...
	private static function getAnswerReportData($question, $answers)
	{
		$data = [];
		...
		return [
			'key' => $question['key'],
			'text' => $question['text'],
			'chartData' => self::createChartData('horizontalBar', $data, 'HORIZONTAL_BAR_OPTIONS'),
			'drawDataInside' => true,
			'hiddenLabel' => true,
	...
	private static function getNumberSalesData($question, $answers, $from, $to, $chartDateFormat, $mysqlDateFormat)
	{
		$results = [];
		$numbers = [];
		$packages = [];
		...
		array_push($results, [
			'key' => $question['package']['key'],
			'text' => $question['package']['text'],
			'chartData' => self::createChartData('line', $packages, null, $dates),
			'drawDataInside' => false,
			'hiddenLabel' => false,
			'dataCount' => count($packages)
		]);

		array_push($results, [
			'key' => $question['voucher']['key'],
			'text' => $question['voucher']['text'],
			'chartData' => self::createChartData('line', $vouchers, null, $dates),
			'drawDataInside' => false,
			'hiddenLabel' => false,
			'dataCount' => count($vouchers)
		]);
